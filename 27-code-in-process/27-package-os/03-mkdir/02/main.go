...
func main() {
	...
	err := os.Mkdir("/tmp/somefolderthatdoesntexist", os.ModePerm)
	if err != nil {
		log.Fatalln("my program broke on mkdir: ", err.Error())
	}

	f, err := os.Create("/tmp/somefolderthatdoesntexist/hello.txt")
	if err != nil {
		log.Fatalln("my program broke", err.Error())
	}
	defer f.Close()

	str := "Put some phrase here. rhidayat was here"
	bs := []byte(str)
